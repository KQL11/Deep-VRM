...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # NOTE: Create a depth variable to track layer depth currently being executed
        depth = 0
        for decoder_layer in self.layers:
            ...
            # NOTE: Increment depth after each layer forward
            depth += 1
            if depth == 16:
                """
                low_level_image_embeds=low_level_image_embeds if pixel_values is not None else None,
                input_ids_ours=input_ids,
                image_mask=image_mask if pixel_values is not None else None,
                """
                low_level_image_embeds = kwargs.get("low_level_image_embeds", None)
                if low_level_image_embeds is not None:
                    image_mask = kwargs.get("image_mask", None)
                    assert image_mask is not None, "The variable 'image_mask' should be provided in kwargs when using low_level_image_embeds."
                    image_mask_2d = image_mask[:, :, 0].to(hidden_states.device)
                    hidden_states[image_mask_2d] = (hidden_states[image_mask_2d] + low_level_image_embeds.to(hidden_states.device)) / 2
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # NOTE: Visual Encoder to get low-level visual features
        self.visual_low_level = Qwen2_5_VisionTransformerPretrainedModel._from_config(config.vision_config)
    ...
    def get_low_level_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        """
        Encodes images into continuous embeddings that can be forwarded to the language model.
        ...
        """
        pixel_values = pixel_values.type(self.visual.dtype)
        image_embeds = self.visual_low_level(pixel_values, grid_thw=image_grid_thw)
        split_sizes = (image_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        image_embeds = torch.split(image_embeds, split_sizes)
        return image_embeds
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            # NOTE: Extract low-level image features
            low_level_image_embeds = self.get_low_level_image_features(pixel_values, image_grid_thw)
            low_level_image_embeds = torch.cat(low_level_image_embeds, dim=0).to(inputs_embeds.device, inputs_embeds.dtype)
