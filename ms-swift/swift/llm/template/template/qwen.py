...
class My_Qwen2VLTemplate(Qwen2VLTemplate):
    version = 'v2'

    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        return inputs
    
register_template(QwenTemplateMeta(MLLMTemplateType.my_qwen2_vl, template_cls=My_Qwen2VLTemplate))
...
class My_Qwen2_5VLTemplate(Qwen2VLTemplate):
    version = 'v2_5'
    norm_bbox = 'none'
...
register_template(QwenTemplateMeta(MLLMTemplateType.my_qwen2_5_vl, template_cls=My_Qwen2_5VLTemplate))


class My_Qwen2_5VLTemplate_expert(Qwen2VLTemplate):
    version = 'v2_5'
    norm_bbox = 'none'
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        # NOTE: custom modification for expert model
        
        temp_ids = copy.deepcopy(input_ids)
        temp_labels = copy.deepcopy(labels)
        new_temp = []
        add_token_num = 0
        for j in range(len(temp_ids)):
            if temp_ids[j] == 151653:
                new_temp.append(temp_ids[j])
                
                new_temp += [151652] + ([151666] * add_token_num) + [151653]
            elif temp_ids[j] == 151655:
                add_token_num += 1
                new_temp.append(temp_ids[j])
            else:
                new_temp.append(temp_ids[j])
        if temp_labels is not None:
            temp_labels = [-100] * (len(new_temp) - len(temp_labels)) + temp_labels
...
register_template(QwenTemplateMeta(MLLMTemplateType.my_qwen2_5_vl_expert, template_cls=My_Qwen2_5VLTemplate_expert))
...
# NOTE
class MyOvis2_5Template(ThinkingTemplate):
    use_model = True
    skip_prompt = False
    support_padding_free = True
...
register_template(QwenTemplateMeta(
    MLLMTemplateType.my_ovis2_5,
    template_cls=MyOvis2_5Template,
    default_system=None,
))
